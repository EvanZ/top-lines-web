...
<template>
  <div class="player-card" :class="{ rsci: isRsci }" :style="{ backgroundImage: `linear-gradient(rgba(17, 24, 39, 0.92), rgba(17, 24, 39, 0.92)), url('${teamLogo}')` }">
    ...
    <table class="stats-table">
      <thead>
        <tr><th>gs</th><th>gp</th><th>mpg</th><th>ts%</th><th>usg%</th><th>ppp</th></tr>
      </thead>
      <tbody>
        <tr>
          <td>{{ p.gs }}</td>
    ...
    <table class="stats-table">
      ...
      <tbody>
        <tr>
          <td :class="pct(p.shooting?.dunka100pctile)">{{ shots.ast_dunk + shots.unast_dunk }}-{{ shots.ast_dunk + shots.unast_dunk + shots.miss_dunk }}</td>
          <td :class="pct(p.shooting?.dunkpctpctile)">{{ p.shooting?.dunkpct?.toFixed(0) }}%</td>
          <td :class="pct(p.shooting?.layupa100pctile)">{{ shots.ast_layup + shots.unast_layup }}-{{ shots.ast_layup + shots.unast_layup + shots.miss_layup }}</td>
          <td :class="pct(p.shooting?.layuppctpctile)">{{ p.shooting?.layuppct?.toFixed(0) }}%</td>
          <td :class="pct(p.shooting?.mida100pctile)">{{ shots.ast_mid + shots.unast_mid }}-{{ shots.ast_mid + shots.unast_mid + shots.miss_mid }}</td>
